...
for product in products:
    try:
        ...
        safety_stock = (product_sales_series['quantity'].max() * max_lead_days) - \
                        (lead_days_sales)
        ...
        if stock_in_hand != 0:
            reorder = {
                ...
                'days_to_expire':0,
                monthly_df['date'].iloc[-2]: [round(monthly_df['quantity'].iloc[-2],2)],
                monthly_df['date'].iloc[-1]: [round(monthly_df['quantity'].iloc[-1],2)],
